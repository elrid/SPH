...
k = 70.0::PositionT -- 70 -- stiffness
mu = 0.03::PositionT -- viscosity coeff
sigma = 10.0::PositionT -- 50.00-- surface tension coeff
douter = 5.0::PositionT -- outer pressure
...
getForce a b = ((pressure - tension) .* (pa .- pb)) .- (viscosity .* (va .- vb)) 
               where 
                 ...
                 w'' = wpoly6dd h r
                 wv  = wvisc h r
                 pressure  = m * k * (da + db - 2 * douter) * ws / (2 * db)
...
wspikyd :: PositionT -> PositionT -> PositionT
wspikyd h r | r<h = 45 * (h - r)^2 / (pi * h^6)
            | otherwise = 0
